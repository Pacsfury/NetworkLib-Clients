...
function nl:sendCmd(opcode, args, is_signal)
    if not self.is_connected or not self.conn then return end
...
function nl:GET(key)
    if not self.is_connected or not self.conn then return nil end
...
function nl:SET(key, val)   self:sendCmd(SET, {key, val}) end
function nl:TEMP(key, val)  self:sendCmd(TEMP, {key, val}) end
function nl:CONST(key, val) self:sendCmd(CONST, {key, val}) end
function nl:SUB(key)        self:sendCmd(SUB, {key}) end

function nl:SIGNAL(...)     self:sendCmd(SIGNAL, {...}, true) end

function nl:update_game_network()
    if not self.is_connected or not self.conn then return nil, "Not Connected" end
